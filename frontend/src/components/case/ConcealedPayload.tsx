    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs h-full flex flex-col justify-between">
      <div>
        <div className="flex items-center gap-2 mb-3">
          <CardTitle icon={AlertOctagon} title="Hidden code detected" tone="violet" info="The app can load or run code that isn't visible in the installed file, for example by downloading a second package or calling methods by name at runtime. Malware does this to hide its real behaviour from scanners." infoAlign="left" />
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {signals.map((sig, idx) => (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md bg-white hover:bg-amber-100 px-2 py-1 text-xs font-medium text-amber-900 border border-amber-300 transition-colors cursor-pointer shadow-2xs"
