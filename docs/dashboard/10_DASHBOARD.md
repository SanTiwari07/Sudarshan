...
The **Analyst Dashboard** is a Single Page Application (SPA) built with React 18, TypeScript, Vite, and Tailwind CSS. It translates complex static decompilation, dynamic Frida traces, and mathematical risk scores into actionable views for fraud analysts, SOC teams, and bank executives.

---

## 2. UI Navigation & Page Routing (`App.tsx`)

The dashboard features a persistent header navigation bar with JWT auth status and role-based guards (`RequireAuth`):

- `/`: Upload & Analysis Launcher (`Upload.tsx`)
- `/executive`: Executive View & Fraud Card (`FraudCard.tsx`)
- `/technical`: Technical SOC View & Workflow Diagram (`TechnicalView.tsx`)
- `/intel`: Threat Intelligence Correlation View (`ThreatIntelView.tsx`)
- `/history`: Historical Analysis Case Store (`History.tsx`)
- `/chat`: AI Investigation Assistant (`InvestigationChat.tsx`)
- `/login`: Analyst JWT Login (`Login.tsx`)
...
- **Risk Badge & FRS Meter**: Visual score dial ($0.0 - 100.0$) and severity band (`CRITICAL`, `HIGH`, `MEDIUM`, `LOW`).
...
Designed for reverse engineers and SOC tier-2/3 analysts:
...
Renders reconstructed `FraudWorkflow` stages:
