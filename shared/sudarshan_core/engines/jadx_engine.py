...
class JadxEngine:
    ...
    def is_available(self) -> bool:
        ...
        if self._available is None:
            ...
            if not binary and self.jadx_path in ("jadx", JADX_PATH):
                # Check workspace local tools directory
                workspace_root = Path(__file__).resolve().parents[3]
                local_bat = workspace_root / "tools" / "jadx" / "bin" / "jadx.bat"
                if local_bat.exists():
                    binary = str(local_bat)
                    self.jadx_path = binary

            self._available = binary is not None
            if not self._available:
                logger.warning(
                    f"[JADX] '{self.jadx_path}' not found in PATH or local tools dir. "
