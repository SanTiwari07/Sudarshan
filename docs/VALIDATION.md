...
3. **Validation Test Suite Protocol**: Documenting the execution of the 285 passing test cases in `backend/tests/`.
...
Verification   (TP/TN Audits) (64 Tests)
```

1. **Determinism Verification**: Replays 9 pinned baseline feature vectors against `risk_engine.py` to assert that zero score drift occurs across codebase updates.
2. **Ground-Truth Matrix**: Verifies that known malicious banking trojans (*Drinik*, *Xenomorph*) trigger `CRITICAL` ($FRS \ge 85.0$) verdicts, while benign and low-risk applications yield `LOW` or `MEDIUM` scores.
3. **Prompt Injection Resilience**: Asserts that malicious prompt injection payloads embedded in APK metadata fail to alter LLM behavior or override risk scoring.
...
| **True Negative (Benign App)** | Clean manifest + Standard permissions + No obfuscation | $0.0 - 29.9$ | **LOW** | Standard Monitoring |
