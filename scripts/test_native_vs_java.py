...
if __name__ == "__main__":
    ...
    # Test Native Hooks from banking_trojan.js
    bundle = Path("shared/sudarshan_core/engines/frida_hooks/banking_trojan.bundle.js").read_text(encoding="utf-8")

    # What if we disable initHooks() call in bundle?
    # Replace "initHooks();" with "// initHooks();"
    bundle_no_java = bundle.replace("initHooks();", "// initHooks();")
    test_spawn("Bundle WITHOUT initHooks() [Only Native hooks]", bundle_no_java)

    # What if we disable installNativeHooks() call in bundle?
    # Replace "installNativeHooks();" with "// installNativeHooks();"
    bundle_no_native = bundle.replace("installNativeHooks();", "// installNativeHooks();")
    test_spawn("Bundle WITHOUT installNativeHooks() [Only Java hooks]", bundle_no_native)
