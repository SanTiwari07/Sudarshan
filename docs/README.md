...
| [**03 — Static Threat Intelligence**](architecture/03_STATIC_THREAT_INTELLIGENCE.md) | Static Analysis & Decompilation | Containerized static engine, MobSF, native `apk_analyzer.py`, APKTool 2.10.0, JADX 1.5.1, `manifest.py` Investigation Manifest, STEI formula. |
...
| [**09 — AI Report Generation**](architecture/09_AI_REPORT_GENERATION.md) | Security Reporting & Export | Executive Fraud Cards, HTML security reports, PDF report exporter, JSON report feed. |
...
| [**11 — Evaluation Strategy**](evaluation/11_EVALUATION.md) | Verification & Testing | Automated test suite in `backend/tests/` (**388 passing tests**), benchmarks, determinism baselines. |
...
- **Verification Metric**: **388 / 388 tests passing clean**.
- **Key Test Modules (in [`backend/tests/`](file:///d:/Projects/Sudarshan%20BOI/backend/tests/))**:
  ...
  - `test_frida_preflight.py`: Verifies Frida attach SELinux preflight execution.
