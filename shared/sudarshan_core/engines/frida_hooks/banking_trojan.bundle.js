...
var eventCounts = {
  ...
  accessibility: 0,
  sms: 0,
  // actual SMS read / write / send / delete ONLY
  overlay: 0,
  // window-type-VERIFIED overlay operations ONLY
  banking: 0,
  // banking-app targeting + high-confidence credential access
  network: 0,
  persistence: 0,
  ...
  app_telemetry: 0,
  // ordinary app behaviour: activity lifecycle, keyboard,
  // generic crypto/keystore/prefs access, non-overlay windows
  notification: 0
  // notification interception (see NOTE at the hook)
};
...
function markOverlayView(view) {
  ...
  if (overlayViewCount >= MAX_TRACKED_OVERLAY_VIEWS) return;
...
function emit(category, data) {
  ...
  var dedupeIdentity = data.ioc || data.url || data.endpoint || data.path || data.property_key || (data.description || "").substring(0, 120);
  ...
  var event = {
    ...
    arguments: data.args || [],
    return_value: data.return_value !== void 0 ? String(data.return_value) : null,
    stacktrace: stack,
...
function initHooks() {
  if (!Java || !Java.available) {
    send({
      type: "error",
      description: "Java bridge unavailable (" + JAVA_BRIDGE_SOURCE + "). On Frida 17+ the Java global was removed; this script must be bundled with frida-java-bridge via frida-compile. NO Java hooks were installed.",
    ...
    Java.perform(function() {
      ...
      try {
        ...
        AccessibilityManager.sendAccessibilityEvent.implementation = function(event) {
          emit("app_telemetry", {
            ...
            severity: "INFO",
            description: "UI accessibility event dispatched (ordinary for any app with a user interface; recorded, not scored)"
          });
      ...
      try {
        ...
        UsageStatsManager.queryEvents.overload("long", "long").implementation = function(begin, end) {
          emit("device_fingerprint", {
            ...
            severity: "HIGH",
            description: "App queried usage-stats events to determine the foreground application (requires PACKAGE_USAGE_STATS)"
          });
        ...
        try {
          WebView.addJavascriptInterface.overload("java.lang.Object", "java.lang.String").implementation = function(obj, name) {
            emit("network", {
              ...
              description: 'WebView exposed a Java object to page JavaScript as "' + (name ? name.toString() : "?") + '" - page script can now call into the app'
            });
            return this.addJavascriptInterface(obj, name);
          };
          registerHook("WebView.addJavascriptInterface");
        } catch (e) {
          reportHookError("WebView.addJavascriptInterface", e.message);
        }
        try {
          ...
          reportHookError("WebView.postUrl", e.message);
        }
      } catch (e) {
      ...
      function sdsnHandleDrain(raw) {
        ...
        for (var i = 0; i < records.length; i++) {
          ...
          emit("network", {
            ...
            description: "WebView page JavaScript issued " + (r.m || "GET") + " " + r.u + " via " + (r.k || "request") + " - invisible to Java networking hooks"
          });
        }
      }
        ...
        SdsnDrainCallback = Java.registerClass({
          ...
          methods: {
            onReceiveValue: function(value) {
              try {
                sdsnHandleDrain(value ? value.toString() : "");
              } catch (e) {
              }
            }
          }
      ...
      function _spoofProperty(keyStr, val) {
        ...
        emit("anti_analysis", {
          ...
          description: "App probed emulator system property: " + keyStr + ' (was "' + valStr + '", spoofed to "' + replacement + '")'
        });
        return replacement;
      }
      try {
      ...
      try {
        ...
        if (spoofedFields.length > 0) {
          emit("harness_action", {
            ...
            description: "SANDBOX ACTION: emulator-identifying Build fields replaced to conceal the analysis environment: " + spoofedFields.join(", ") + " (performed by the harness, not by the application)"
          });
        }
      ...
      try {
        ...
        BufferedReader.readLine.overload().implementation = function() {
          ...
          var suppressed = 0;
          while (line !== null && (line.indexOf("frida") !== -1 || line.indexOf("gum-js") !== -1) && suppressed < MAX_SUPPRESSED_LINES) {
            suppressed++;
            line = this.readLine();
          }
          if (suppressed > 0) {
      ...
      try {
        var System = Java.use("java.lang.System");
        System.exit.implementation = function(code3) {
          emit("anti_analysis", {
            hook: "System.exit",
            class_name: "java.lang.System",
            severity: "CRITICAL",
      ...
      try {
        var Runtime2 = Java.use("java.lang.Runtime");
        Runtime2.exit.implementation = function(code3) {
          emit("anti_analysis", {
            hook: "Runtime.exit",
            class_name: "java.lang.Runtime",
            severity: "CRITICAL",
          ...
          try {
            if (type && type.indexOf("application/vnd.android.package-archive") >= 0) {
              ...
              emit("persistence", {
                ...
                severity: "CRITICAL",
                path: target,
                description: "Application asked Android to install a package: " + target + " (install REQUESTED - completion not observable here)"
              });
            }
          } catch (e2) {
          }
