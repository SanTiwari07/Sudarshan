  ...
  return (
    ...
    >
      <div className="aspect-[9/16] w-full max-h-[240px] bg-slate-50 relative overflow-hidden">
        {loading && (
          <div className="absolute inset-0 animate-pulse bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center text-[12px] text-slate-450 font-mono">
            LOADING…
          </div>
        )}
        {src && (
        ...
        <div className="absolute top-1.5 left-1.5 flex flex-wrap gap-1">
          <span className="text-[11px] font-mono font-bold px-1 py-0.2 rounded bg-slate-900/80 text-white">
            {String(index + 1).padStart(2, '0')}
          </span>
          {evidenceId && (
            <span className="text-[11px] font-mono font-bold px-1 py-0.2 rounded bg-blue-700 text-white">{evidenceId}</span>
          )}
          {mitre && (
            <span className="text-[11px] font-mono font-bold px-1 py-0.2 rounded bg-slate-900/90 text-white">{mitre}</span>
          )}
        </div>
      </div>
      <div className="p-2 border-t border-slate-150 space-y-0.5 bg-slate-50/20">
        ...
        <div className="text-[13px] font-bold text-slate-800 line-clamp-1 leading-tight">{description}</div>
      </div>
    </button>
  );
}
