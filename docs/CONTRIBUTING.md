...
All PRs must maintain 100% test suite compliance (**583 / 583 tests collected** across `tests/` and `backend/tests`). Never break existing test assertions or compromise the **Determinism Invariant**.
...
2. Ensure all automated pytest test cases pass (`pytest tests/ backend/tests` - **583 tests collected**).
