...
def classify_family(flags: StaticAnalysisFlags) -> tuple[str, str]:
    ...
    """
    # Drinik before Xenomorph: both can match accessibility+SMS+banking, but
    # Drinik requires DexClassLoader (more specific Indian-banking trojan signal).
    if flags.targets_indian_banks and "DexClassLoader" in flags.dangerous_apis_found:
        return "Drinik", "Banking Target + Dynamic Code Loading = Drinik-pattern"
    elif flags.has_accessibility_abuse and flags.has_sms_read_write and flags.targets_indian_banks:
