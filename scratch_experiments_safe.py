...
async def run_experiment(apk_name, apk_path, mode):
    ...
    try:
        async with AsyncClient(timeout=120) as client:
            ...
            r = await client.post(f"{BACKEND_URL}/api/v1/analyze/async", headers={'Authorization': 'Bearer '+token}, files={'file': (apk_path.split('\\')[-1], open(apk_path, 'rb'), 'application/vnd.android.package-archive')}, timeout=120)
            ...
            while True:
                try:
                    r = await client.get(f"{BACKEND_URL}/api/v1/status/{job_id}", headers={'Authorization': 'Bearer '+token}, timeout=30)
                    st = r.json()
                    ...
                    print("ReadError during polling:", e)
                time.sleep(10)
...
async def main():
    for apk_name, apk_path in APKS.items():
        for mode in MODES:
            if not os.path.exists(f"result_{apk_name}_{mode}.json"):
