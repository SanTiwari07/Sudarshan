  ...
  return (
    <li className="rounded border border-slate-200">
      <div className="flex items-center justify-between gap-2 px-2.5 py-1.5 bg-slate-50/70 border-b border-slate-200">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            className="text-[11px] px-2 py-0.5 rounded border border-slate-300 text-slate-700 hover:bg-white"
          >
          ...
          <button
            type="button"
            onClick={download}
            className="text-[11px] px-2 py-0.5 rounded border border-slate-300 text-slate-700 hover:bg-white inline-flex items-center gap-1"
          >
      ...
      {signals.length > 0 && (
        <div className="px-2.5 py-1.5 flex flex-wrap gap-1 border-b border-slate-100">
          {signals.map((s) => (
            <span
              key={s}
              className="text-[10px] bg-rose-50 border border-rose-200 text-rose-800 rounded px-1.5 py-0.5"
            >
              {s}
            </span>
          ))}
      ...
      {expanded && (
        ...
        // set as innerHTML.
        <pre className="px-2.5 py-2 max-h-72 overflow-auto text-[10px] font-mono text-slate-700 whitespace-pre-wrap break-all bg-slate-50/40">
          {payload.html}
        </pre>
      )}
    </li>
